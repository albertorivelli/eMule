...
namespace eMule
{
    public static class Opcodes
    {
        ...
        public const int EMULE_PROTOCOL = 0x01;
        // MOD Note: end
        public const int EDONKEYVERSION = 0x3C;
        public const int KADEMLIA_VERSION1_46c = 0x01; /*45b - 46c*/
        public const int KADEMLIA_VERSION2_47a = 0x02; /*47a*/
        public const int KADEMLIA_VERSION3_47b = 0x03; /*47b*/
        public const int KADEMLIA_VERSION5_48a = 0x05; // -0.48a
        public const int KADEMLIA_VERSION6_49aBETA = 0x06; // -0.49aBETA1, needs to support: OP_FWCHECKUDPREQ (!), obfuscation, direct callbacks, source type 6, UDP firewallcheck
        public const int KADEMLIA_VERSION7_49a = 0x07; // -0.49a needs to support OP_KAD_FWTCPCHECK_ACK, KADEMLIA_FIREWALLED2_REQ
        public const int KADEMLIA_VERSION8_49b = 0x08; // TAG_KADMISCOPTIONS, KADEMLIA2_HELLO_RES_ACK
        public const int KADEMLIA_VERSION9_50a = 0x09; // handling AICH hashes on keyword storage
        public const int KADEMLIA_VERSION = 0x09; // Change CT_EMULE_MISCOPTIONS2 if Kadversion becomes >= 15 (0x0F)
        public const int PREFFILE_VERSION = 0x14;   //<<-- last change: reduced .dat, by using .ini
        public const int PARTFILE_VERSION = 0xe0;
        public const int PARTFILE_SPLITTEDVERSION = 0xe1;
        public const int PARTFILE_VERSION_LARGEFILE = 0xe2;
        public const int SOURCEEXCHANGE2_VERSION = 4;       // replaces the version sent in MISC_OPTIONS flag fro SX1

        public const int CREDITFILE_VERSION = 0x12;
        public const int CREDITFILE_VERSION_29 = 0x11;
        public const long COMPILE_DATE = 0;
        public const long COMPILE_TIME = 0;
#if DEBUG
        public const string EMULE_GUID = "EMULE-{4EADC6FC-516F-4b7c-9066-97D893649570}-DEBUG";
#else
public const string EMULE_GUID = "EMULE-{4EADC6FC-516F-4b7c-9066-97D893649570}";
#endif

        public static uint SEC2MS(uint sec) { return ((sec) * 1000); }
        public static uint MIN2MS(uint min) { return SEC2MS((min) * 60); }
        ...
        public const int KADEMLIADISCONNECTDELAY = 20;	//20 mins
        ...
        public const int KADEMLIAFIREWALLCHECKS = 4;			//Firewallcheck Request at a time
        ...
        public const byte KADEMLIA_BOOTSTRAP_REQ_DEPRECATED = 0x00;   // <PEER (sender) [25]>
        public const byte KADEMLIA2_BOOTSTRAP_REQ = 0x01;   //

        public const byte KADEMLIA_BOOTSTRAP_RES_DEPRECATED = 0x08;   // <CNT [2]> <PEER [25]>*(CNT)
        public const byte KADEMLIA2_BOOTSTRAP_RES = 0x09;   //

        public const byte KADEMLIA_HELLO_REQ_DEPRECATED = 0x10;   // <PEER (sender) [25]>
        public const byte KADEMLIA2_HELLO_REQ = 0x11;   //

        public const byte KADEMLIA_HELLO_RES_DEPRECATED = 0x18;   // <PEER (receiver) [25]>
        public const byte KADEMLIA2_HELLO_RES = 0x19;   //

        public const byte KADEMLIA_REQ_DEPRECATED = 0x20;   // <TYPE [1]> <HASH (target) [16]> <HASH (receiver) 16>
        public const byte KADEMLIA2_REQ = 0x21;   //

        public const byte KADEMLIA2_HELLO_RES_ACK = 0x22; // <NodeID><uint8 tags>

        public const byte KADEMLIA_RES_DEPRECATED = 0x28;   // <HASH (target) [16]> <CNT> <PEER [25]>*(CNT)
        public const byte KADEMLIA2_RES = 0x29; //

        public const byte KADEMLIA_SEARCH_REQ = 0x30;   // <HASH (key) [16]> <ext 0/1 [1]> <SEARCH_TREE>[ext]
        //public const byte UNUSED = 0x31;   // Old Opcode, don't use.
        public const byte KADEMLIA_SEARCH_NOTES_REQ = 0x32; // <HASH (key) [16]>
        public const byte KADEMLIA2_SEARCH_KEY_REQ = 0x33;  //
        public const byte KADEMLIA2_SEARCH_SOURCE_REQ = 0x34;   //
        public const byte KADEMLIA2_SEARCH_NOTES_REQ = 0x35;    //

        public const byte KADEMLIA_SEARCH_RES = 0x38;   // <HASH (key) [16]> <CNT1 [2]> (<HASH (answer) [16]> <CNT2 [2]> <META>*(CNT2))*(CNT1)
        //public const byte UNUSED = 0x39;	// Old Opcode, don't use.
        public const byte KADEMLIA_SEARCH_NOTES_RES = 0x3A; // <HASH (key) [16]> <CNT1 [2]> (<HASH (answer) [16]> <CNT2 [2]> <META>*(CNT2))*(CNT1)
        public const byte KADEMLIA2_SEARCH_RES = 0x3B;  //

        public const byte KADEMLIA_PUBLISH_REQ = 0x40;  // <HASH (key) [16]> <CNT1 [2]> (<HASH (target) [16]> <CNT2 [2]> <META>*(CNT2))*(CNT1)
        //public const byte UNUSED = 0x41;	// Old Opcode, don't use.
        public const byte KADEMLIA_PUBLISH_NOTES_REQ_DEPRECATED = 0x42; // <HASH (key) [16]> <HASH (target) [16]> <CNT2 [2]> <META>*(CNT2))*(CNT1)
        public const byte KADEMLIA2_PUBLISH_KEY_REQ = 0x43; //
        public const byte KADEMLIA2_PUBLISH_SOURCE_REQ = 0x44;  //
        public const byte KADEMLIA2_PUBLISH_NOTES_REQ = 0x45;   //

        public const byte KADEMLIA_PUBLISH_RES = 0x48;  // <HASH (key) [16]>
        //public const byte UNUSED = 0x49;	// Old Opcode, don't use.
        public const byte KADEMLIA_PUBLISH_NOTES_RES_DEPRECATED = 0x4A; // <HASH (key) [16]>
        public const byte KADEMLIA2_PUBLISH_RES = 0x4B; //
        public const byte KADEMLIA2_PUBLISH_RES_ACK = 0x4C; // null

        public const byte KADEMLIA_FIREWALLED_REQ = 0x50;   // <TCPPORT (sender) [2]>
        public const byte KADEMLIA_FINDBUDDY_REQ = 0x51;    // <TCPPORT (sender) [2]>
        public const byte KADEMLIA_CALLBACK_REQ = 0x52; // <TCPPORT (sender) [2]>
        public const byte KADEMLIA_FIREWALLED2_REQ = 0x53;  // <TCPPORT (sender) [2]><userhash><connectoptions 1>

        public const byte KADEMLIA_FIREWALLED_RES = 0x58;   // <IP (sender) [4]>
        public const byte KADEMLIA_FIREWALLED_ACK_RES = 0x59;   // (null)
        public const byte KADEMLIA_FINDBUDDY_RES = 0x5A;    // <TCPPORT (sender) [2]>

        public const byte KADEMLIA2_PING = 0x60;    // (null)
        public const byte KADEMLIA2_PONG = 0x61;    // (null)

        public const byte KADEMLIA2_FIREWALLUDP = 0x62;	// <errorcode [1]><UDPPort_Used [2]>
